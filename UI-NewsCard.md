...
> **Файлы:** `src/components/NewsCard.tsx`, `src/components/SentimentTooltip.tsx`, `src/components/NewsReactionChart.tsx`, `src/components/CandleChart.tsx`
> **Версия:** 8.2.0
> **Дата:** 2026-08-22
> **Изменения в v8.2.0:** Компактные поля графика (TZ-3.8) и тикер в подписи под графиком (TZ-3.9).
...
### 2.8.1 График реакции цены (TZ-3 / TZ-3.8 / TZ-3.9)

Показывается только в карточках, где call-site передал `showChart={true}` (например, «Вся лента» и страница `/news`).

**Компоненты:** `NewsReactionChart` → `CandleChart`.

**Размеры:**
- Высота контейнера: `180px`.
- Свечной график занимает `62%` высоты, объёмы — `14%`.
...
```
- `left: 30` — минимальный gutter для подписей цен оси Y (~27px при `fontSize: 10`).
- `right: 6` — справа ничего не выводится.
- Результат: поле свечей в карточке 243px растёт с ~171px до ~207px (+21%).

**Подпись под графиком (TZ-3.9):**
```
{TICKER} · {Биржа} · время биржи
```
- Тикер берётся как `activeInstrument.symbol.split('@')[0]` — консистентно с табами инструментов.
- Тикер стилизован `text-gray-400 font-semibold`, чтобы считывался первым.
- Если новость вне сессии — добавляется `· вне сессии — показан ближайший день {date}`.
- Пример: `TSLA · NASDAQ · время биржи` или `SBER · Мосбиржа · время биржи · вне сессии — показан ближайший день 2026-08-20`.

**Переключение инструментов:**
- Если инструментов несколько — табы по тикеру над графиком.
- `activeInstrument` реактивен; подпись обновляется при переключении.

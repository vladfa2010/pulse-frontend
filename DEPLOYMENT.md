...
>
> **⚡ СТАТУС:** работают **две идентичные параллельные среды** — осознанное состояние, не миграция:
...
>   Там же живёт **tgparser-web** — критичный источник телеграм-новостей для обеих сред.
...
>
> Полная актуальная версия документа (VPS-процедуры, бэкапы, откаты) — в репозитории
> `pulse-backend` (`DEPLOYMENT.md`). Эта копия в `pulse-frontend` дополнена разделами
> RSS Агрегация, Smart Tag Matching, Database Schema.
...
> **Вторая среда (с 2026-09-03):** VPS 155.212.216.142 (Россия, без VPN, docker compose:
> caddy + backend + postgres) — полная копия стека. Домен `pulse.inside-trade.ru`
> смотрит на VPS; Render-контур доступен по onrender-адресам и обновляется автодеплоем
> из `main`. Подробности и процедуры — в `pulse-backend/DEPLOYMENT.md`.

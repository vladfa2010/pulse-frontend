...
import { BROKER_META, type BrokerPosition } from '@/types/portfolio'

export default function ConsolidatedTable({ positions }: { positions: BrokerPosition[] }) {
  if (positions.length === 0) {
    return (
      <div className="px-6 py-8 text-center text-[13px] text-[#6B7280]">
        Позиций пока нет — добавьте портфель или дождитесь синхронизации
      </div>
    )
  }

  const max = Math.max(...positions.map(p => p.weightPct ?? 0), 1)
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full border-collapse min-w-[860px]">
        <thead>
          <tr>
            {['Тикер', 'Компания', 'Кол-во', 'Средняя', 'Текущая', 'Стоимость', 'P&L', 'Где', 'Вес'].map((h, i) => (
              <th
                key={h}
        ...
        <tbody>
          {positions.map(p => {
            ...
            return (
              <tr key={p.ticker} className="transition-colors hover:bg-white/[0.025]">
                ...
                <td className="py-3 px-6 text-right text-[13px] text-white">
                  <div className="flex flex-wrap gap-1 justify-end">
                    {(p.sources || []).map(s => {
                      const meta = BROKER_META[s.broker]
                      return (
                        <span
                          key={s.broker}
                          className="inline-flex items-center gap-1 text-[9px] font-bold uppercase tracking-[0.05em] px-[7px] py-[2px] rounded-md"
                          style={{
                            color: meta.color,

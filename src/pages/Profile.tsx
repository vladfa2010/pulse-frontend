...
import {
  ...
  Unlink, Moon, Mail, Check, Sparkles, Tag, AlertTriangle, Lock, X,
} from 'lucide-react'
...
export default function Profile() {
  ...
  const [freezeBannerClosed, setFreezeBannerClosed] = useState(() => {
    const closed = localStorage.getItem('freezeBannerClosed')
    if (!closed) return false
    return Date.now() - parseInt(closed, 10) < 24 * 60 * 60 * 1000
  })
  ...
  useEffect(() => {
    ...
    api.get('/plans')
      .then(data => setAllPlans(data.plans || data || []))
      .catch(() => setAllPlans([]))
  }, [isLoggedIn])
  ...
  const closeFreezeBanner = () => {
    localStorage.setItem('freezeBannerClosed', String(Date.now()))
    setFreezeBannerClosed(true)
  }

  const deleteTagFromBanner = async (tagId: string) => {
  ...
  const lostFeatureLabels = useMemo(() => {
    if (!tariff || tariff.subscription.active) return []
    const currentFeatures = tariff.plan.features || {}
    const freeFeatures = freePlan?.features || { telegram: false, push: false, ai_summary: false, alerts: false, priority: 'normal' }
    const labels: Record<string, string> = {
      telegram: 'Telegram-дайджест',
      push: 'Push-уведомления',
      ai_summary: 'AI-саммари по портфелю',
      alerts: 'Sentiment-алерты',
      priority: 'Приоритетная доставка',
      early_delivery: 'Ранняя доставка',
      custom_thresholds: 'Кастомные пороги',
      club_access: 'Club доступ',
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: '#0a0a0a' }}>
      ...
      <div className="max-w-[900px] mx-auto px-6 md:px-12 pb-20">
        {(() => {
          if (!tariff || freezeBannerClosed) return null
          ...
          const shouldShow = (hasFrozen || activeOverLimit) && (!tariff.subscription.active || tariff.subscription.plan === 'free')
          ...
          return (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -12 }}
              className="mb-6 rounded-2xl p-5 md:p-6 relative overflow-hidden"
              style={{
                background: 'rgba(255, 255, 255, 0.02)',
                backdropFilter: 'blur(12px) saturate(180%)',
                WebkitBackdropFilter: 'blur(12px) saturate(180%)',
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center justify-between gap-3">
                          ...
                          <button
                            onClick={() => deleteTagFromBanner(tag.tag_id)}
                            disabled={isDeleting}
          ...
          {activeTab === 'profile' && (
            ...
            >
              ...
              <GlassCard accentColor="#10B981">
                  ...
                  {(stats?.user_tags_count ?? portfolio.length) === 0 && (
                  ...
                  <div className="flex flex-wrap gap-2">
                    {portfolio.map(tag => (
                      <div
                        ...
                        style={{
                          background: tag.is_frozen
                            ? 'rgba(255, 255, 255, 0.03)'
                            : 'rgba(0, 212, 255, 0.06)',
                          border: tag.is_frozen
                            ? '1px solid rgba(255, 255, 255, 0.08)'
                            : '1px solid rgba(0, 212, 255, 0.12)',
                          color: tag.is_frozen ? '#9CA3AF' : '#00D4FF',
                        }}
                      >
                        {tag.is_frozen ? (
                        ...
                        {tag.is_frozen ? (
                          <Link
                            to="/pricing"
                            className="ml-1 text-[#00D4FF] hover:underline text-xs whitespace-nowrap"
                          >
                            Восстановить Premium
                          </Link>
                        ) : (
                          <button
            ...
            >
              {loadingTariff ? (
                ...
                <>
                  ...
                  {/* Lost features (when not active) */}
                  {lostFeatureLabels.length > 0 && (
                    <GlassCard accentColor="#EF4444">
                      <div className="flex items-center gap-3 mb-4">
                        ...
                        <div>
                          <h3 className="text-lg font-semibold text-white">Недоступные функции</h3>
                          <p className="text-xs text-[#6B7280]">На Free эти опции не работают</p>
                        </div>
                      </div>
                      <div className="space-y-2 mb-5">
                        {lostFeatureLabels.map(f => (

...
- Видимый фон-слот хоста (первая версия ТЗ-111: `background`/`border` у `.cascade-stack-host`) **отклонён владельцем** — под короткой карточкой читалась фантомная карточка максимального размера. Карточка чистая: ни фона, ни рамки у хоста нет, под короткой картой — просто фон страницы.
- Cyan-свечение каскада (ТЗ-108) вместо `::before` на хосте (`inset:0` → свечение в пустоту под короткой картой) перенесено на обёртку карточки: `.cascade-stack-host--stacked .cascade-card-wrap::before` (z −2 внутри z-[1]-контекста обёртки) — свечение обнимает только карточку. Порядок отрисовки: свечение → слои стопки (z −1, ТЗ-109) → карточка. Хвост прилежит к низу своей карточки.

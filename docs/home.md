...
- **Настройки (утверждённый мокап, все явно):** `variant "stack"`, `scrollLength 1`, `peek 26`, `scaleStep 0.07`, `blur 4`, `dim 0.28`, `smooth 0.16`, `depth 3`, `cardWidth 880`, `cardHeight 0.324` (дефолт 0.68 — ниже на ~52% по мокапу; +20% от 0.27), `borderRadius 16`, `perspective 1400`. Если покажется слишком длинным пробег — крутить `scrollLength` (0.7–0.8), код не трогать.

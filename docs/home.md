...
- **Сцена:** стопка из 5 карточек-инструментов (eyebrow 01–05 с акцентными цветами, заголовок, текст; картинок нет). Карточки наезжают друг на друга по скроллу: предыдущая уменьшается/темнеет/блюрится, край подглядывает на 26px. Счётчик `01/05` и прогресс-рейл внизу сцены наследуют белый цвет страницы. Sticky-сцена: ранвей 500vh (`scrollLength 1` × 4 перехода + 100vh вход).
...
- **⚠️ Локальные патчи вендорного файла (ТЗ-77/78/79):** `src/components/react-bits/scroll-stack.tsx` перекрашен вручную (класс `.dark` в проект нигде не выставляется, поэтому `dark:`-варианты из реестра мёртвые — карточка была бы светлой). Патчи: (1) тёмная карточка (`border-white/15 bg-[#0E0E0E]`), типографика, акцентное свечение, номер `text-white/50`, `aria-label "Инструменты PULSE"` (ТЗ-77); (2) prop `header` — pinned-заголовок внутри sticky-сцены (`top: calc(4rem + env(safe-area-inset-top) + 20px)`, `z-[60]`, `pointer-events-none`; вешается на контейнер колоды `transform: translateY(-14vh)` — зазор заголовок→карточка ~35% от прежнего) (ТЗ-78); (3) контент карточки прижат к верху (ТЗ-79). Математика (`pose/glide/paint/measure`), ранвей, reduced-motion (режим `calm`), прогресс-рейл/счётчик — не тронуты. При переустановке из реестра (`npx shadcn add … scroll-stack-tw`) все патчи **затрутся** — восстановить по этому разделу.

...
11. **HomeScrollStack** — ScrollStack «Инструменты»: стопка из 5 карточек, наезжают друг на друга по скроллу (только гостям, сразу под hero — ТЗ-77). Детали — раздел «ScrollStack» ниже.
...
- **Карточка:** тёмная тема PULSE — поверхность `#0E0E0E`, рамка `white/15`, радиус 16px, сверху акцентное свечение цвета eyebrow (`radial-gradient(120% 90% at 50% -20%, ${accent}26 …)`). Типографика: заголовок `text-[clamp(22px,2.6vw,30px)] leading-[1.15] tracking-tight text-balance` (без `max-w`), текст `text-[14px] leading-[1.6] max-w-[64ch] text-[#9CA3AF]`, контейнер `gap-[10px] px-8 py-6 max-sm:p-[18px]`, номер `text-white/50`.
- **⚠️ Локальные патчи вендорного файла (ТЗ-77):** `src/components/react-bits/scroll-stack.tsx` перекрашен вручную (класс `.dark` в проект нигде не выставляется, поэтому `dark:`-варианты из реестра мёртвые — карточка была бы светлой). Патчи: тёмная карточка (`border-white/15 bg-[#0E0E0E]`), типографика заголовка/текста/контейнера, акцентное свечение, номер `text-white/50`, `aria-label "Инструменты PULSE"`. Математика (`pose/glide/paint/measure`), reduced-motion (режим `calm`), прогресс-рейл/счётчик — не тронуты. При переустановке из реестра (`npx shadcn add … scroll-stack-tw`) перекраска **затрётся** — восстановить по этому разделу.

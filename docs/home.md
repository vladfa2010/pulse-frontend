...
- количество тегов превышает лимит текущего тарифа (`to_remove > 0`);
...
| Поле | Тип | Описание |
|------|-----|----------|
| `current_plan` | string | ID текущего тарифа |
| `plan_name` | string | Отображаемое имя тарифа |
...
- **Кнопка сохранения**:
  ...
  - при нажатии отправляется `POST /api/user/select-active-tags` с телом `{ activeTagIds: [...] }` — массив `id` (не `tag_id`) всех активных (не замороженных) тегов, отображаемых в баннере.

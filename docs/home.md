...
8. `PublicInfoVolume` — «Объём информации» эталонного аккаунта (только гостям, сразу под «Суперсилой» — ТЗ-56). Вёрстка 1:1 с профилем (glass-карта, зелёный акцент #10B981, сетка 2×2), данные — `GET /api/public/efficiency` (публичный, кэш 60 с на бэке, эталонный аккаунт через env `PUBLIC_EFFICIENCY_EMAIL`). Подзаголовок про эталонную подборку. Ошибка/404 → блок молча скрыт.

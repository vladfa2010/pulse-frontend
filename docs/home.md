...
- **Сцена:** стопка из 5 карточек-инструментов (eyebrow 01–05 с акцентными цветами, заголовок, текст; картинок нет). Карточки наезжают друг на друга по скроллу: предыдущая уменьшается/темнеет/блюрится, край подглядывает на 26px. Счётчик `01/05` и прогресс-рейл под карточкой (24px от низа сцены). **Компактная сцена (ТЗ-80):** высота sticky-сцены ~68vh (`navbar + 20px + кегль заголовка + 20vh зазор + карточка 27vh + 64px на рейл`), не `100vh` — мёртвого хвоста после последней карточки нет. Ранвей = высота сцены + 400vh (`scrollLength 1` × 4 перехода), входа 100vh нет. Прогресс считается от высоты сцены, не вьюпорта (`measure()`, span = root − stage).

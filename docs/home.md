...
- **Сцена:** стопка из 5 карточек-инструментов (eyebrow 01–05 с акцентными цветами, заголовок, текст; картинок нет). Карточки наезжают друг на друга по скроллу: предыдущая уменьшается/темнеет/блюрится, край подглядывает на 26px. Счётчик `01/05` и прогресс-рейл под карточкой (24px от низа сцены). **Компактный ранвей (ТЗ-80, правка 2026-09-10):** высота контентной части ~74vh (`navbar + 20px + кегль заголовка + 21vh зазор + карточка ~32vh + 64px на рейл`) — из неё считается высота секции (`stageH + 400vh`), поэтому хвост после финала короче, чем в исходном ТЗ-77. Сама sticky-сцена снова `100vh` — рейл/счётчик стоят у нижнего края **экрана** (`bottom-6`), а не под карточкой. Прогресс считается от высоты сцены (`measure()`, span = root − stage).

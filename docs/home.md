...
5а. `DemoFeedCarousel` — демо-лента с графиками (только гостям, перед общей лентой — ТЗ-59). Лента демо-аккаунта (`GET /api/public/demo-feed?limit=50&page=N`, кэш 60 с по page; формат ответа идентичен `/api/news/global`), карточки `NewsCard` с `showChart` — график реакции рынка запрашивается только когда карточка во вьюпорте. Структура — как `GlobalNewsCarousel` (infinite scroll по sentinel, `dedupById`), FLIP-анимация не подключена. Ошибка/пусто → блок скрыт.
5б. `DemoTagsRow` — демо-теги под поиском в hero (только гостям — ТЗ-59). Чипы в стиле компонента `Tag` (те же цвета по типам), read-only: клик по любому → модалка регистрации (`openAuthModal('register')`). Источник — `GET /api/public/demo-tags` (кэш 60 с). Ошибка/пусто → блок скрыт. Демо-аккаунт: env `PUBLIC_DEMO_EMAIL` (дефолт `vladfa@ya1.ru`) — владелец наполняет тегами сам, контент лендинга правится без деплоя; замороженные теги не отдаются.
...
8. `PublicInfoVolume` — «Объём информации» эталонного аккаунта (только гостям, сразу под «Суперсилой» — ТЗ-56). Вёрстка 1:1 с профилем (glass-карта, зелёный акцент #10B981, сетка 2×2), данные — `GET /api/public/efficiency` (публичный; кэш на сутки, ленивый пересчёт + прогрев после boot — ТЗ-57, эталонный аккаунт через env `PUBLIC_EFFICIENCY_EMAIL`). Подзаголовок «обновляются ежедневно» соответствует реальности кэша. Ошибка/404 → блок молча скрыт.
9. `SentimentChartCard` — индекс настроения. Блок вынесен в модульный компонент `HomeSentimentIndex` (тот же JSX, Suspense + скелетон). Авторизованным — после `PublicInfoVolume`; гостям — сразу после `CalendarBlock` (календарь инвестора).
10. `TelegramConnectBanner` — промо-баннер Telegram.
11. `PopularTagsSlider` — популярные теги. Авторизованным — после Telegram-баннера; гостям — самый низ страницы (после календаря).
12. Блок «Портфель инвестиционно.рф» (только авторизованным — с гостевой главной убран).
13. `FeaturesCarousel` — «Что Pulse делает вместо вас», карусель из 7 карточек поверх общей оболочки `NewsCarousel` (стрелки по 450px, нативный скролл/свайп, боковые затемнения; карточки 210px). Только гостям, сразу под hero. Первые 3 карточки («Хватит читать перепечатки», «Цена уже здесь», «Не верьте заголовку на слово») — видны без скролла; порядок — часть смысла. Компонент: `src/components/home/FeaturesCarousel.tsx`. Старая статичная сетка `FeatureCard` из Home.tsx удалена.
13а. `GlobalSummary isPublic` — «Пульс рынка»: ИИ-саммари всей ленты (до 200 новостей за 6 часов) для гостей, сразу под каруселью. Публичный режим компонента `GlobalSummary`: ходит в `GET /api/public/summary-global`, который отдаёт **только свежий кэш** (генерацию LLM не триггерит — прогрев кэша делает cron: warm-up через 3 мин после boot + каждые 6 ч). Кнопки «Обновить» у гостей нет; кэша нет/ошибка → блок молча скрыт. Вёрстка 1:1 с авторизованной версией (ширина как у карусели, `px-6`).
14. `CalendarBlock` — календарь инвестора.
15. `MarketPulseMini` — мини-блок «Пульс рынка» (тепловая карта активности). Публичный, скрывается при отсутствии новостей. Клик → `/activity-map?scope=all&scale=year`.
16. Второй hero «Ваши инструменты» — заголовок стилем основного hero (кегль `clamp(32px, 5.33vw, 64px)`, градиент + голубой курсив, та же анимация framer-motion). Только гостям, размещён над «Пульсом рынка».
17. CTA «Регистрация» — чёрная кнопка (поверхность `#0E0E0E`, белый текст, pill-радиус) с эффектом BorderGlow (reactbits, MIT; порт: `src/components/BorderGlow.tsx`): светящаяся рамка из палитры cyan/зелёный/фиолет следует за курсором, а при появлении во вьюпорте (IntersectionObserver, порог 0.6) свечение пробегает по рамке; пробег повторяется при каждом возврате на экран, не стартует под курсором и пока предыдущий пробег не доиграл. Клик → `openAuthModal('register')` — модалка авторизации сразу на табе создания аккаунта (аналитика `open_auth_modal` срабатывает сама). Только гостям, последний элемент гостевой страницы (после `PopularTagsSlider`).

...
15. `MarketPulseMini` — мини-блок «Пульс рынка» (тепловая карта активности). Публичный, скрывается при отсутствии новостей. Клик → `/activity-map?scope=all&scale=year`. Позиция: гостям — сразу под каруселью «Что Pulse делает вместо вас» (под hero); авторизованным — перед календарём. Ленивый маунт через `LazyRender` в обеих точках (ТЗ-48).
16. Второй hero «Ваши инструменты» — заголовок стилем основного hero (кегль `clamp(32px, 5.33vw, 64px)`, градиент + голубой курсив, та же анимация framer-motion). Только гостям, размещён над «Пульсом рынка» (вместе с ним под каруселью).

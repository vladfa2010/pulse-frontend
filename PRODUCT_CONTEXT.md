...
2. Задеплоить
3. **Git commit с описанием: `git add -A && git commit -m "type: description"`**
4. **Указать пользователю: какие файлы обновлены + git commit hash**
5. **Обновить DESIGN_SPEC.md если логика изменилась**
6. **Обновить PRODUCT_CONTEXT.md если договорённости изменились**

**❌ ЗАПРЕЩЕНО: делать изменения без git commit и без указания пользователю**

...
### ❌❌❌ КРИТИЧЕСКОЕ ПРАВИЛО: ТОЛЬКО РЕАЛЬНЫЕ НОВОСТИ — НИКАКИХ MOCK
- **НИКОГДА НЕ используем фейковые/mock новости на фронтенде**
- Все новости ТОЛЬКО с бэкенда через API (GET /api/news)
- НЕ генерируем шаблонные заголовки, НЕ создаём фейковые данные
- RSS агрегатор — единственный источник новостей (32 источника, каждые 15 мин)
- UNIQUE(url) на таблице news предотвращает дубликаты
- Карусель "Это вы ещё не видели" — реальные непрочитанные из PostgreSQL
...
### Новое правило: Все попапы — backdrop blur
- **Концепция:** При открытии любого модального окна фон размывается (`backdrop-blur-sm`)
- **Эффект:** Создаёт ощущение глубины, фокус на попапе
- **Backdrop:** `bg-black/70` — тёмный полупрозрачный слой
- **Применимо ко всем попапам:** AuthModal, PremiumPromptModal, и будущим
...
| # | Задача | Приоритет | Статус |
|---|--------|-----------|--------|
| 6 | **Error Boundaries** — react-error-boundary, не падает вся страница | P1 | Запланировано |
| 7 | **Structured logging** — winston/pino вместо console.log | P1 | Запланировано |
| 8 | **Миграции БД** — node-pg-migrate вместо ручных ALTER | P1 | Запланировано |
| 9 | **Мониторинг** — Sentry (ошибки) + UptimeRobot (доступность) | P1 | Запланировано |
| 10 | **API документация** — Swagger/OpenAPI для всех endpoints | P1 | Запланировано |
...
**Спецификации компонентов:** `AUTH_MODAL_SPEC.md` — полный чертёж AuthModal  
**Автотесты:** `AUTO_TESTS.md` — 6 сценариев + 9 критических проверок (PASS/FAIL перед деплоем)
...
- ✅ **Только НЕВИДЕННОЕ:** Каждая новость показывается только 1 раз. Прочитанные исключаются из ленты автоматически (таблица `user_news_reads`). Frontend отмечает прочитанные через POST /api/news/:id/read

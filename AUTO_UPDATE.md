...
3. Если доступна более новая версия — показывается модалка «Доступно обновление».
4. На Android при нажатии «Обновить» нативный плагин скачивает APK через `DownloadManager` и запускает системный диалог установки.
5. На web (и прочих платформах) открывается страница релиза в браузере.
...
- Логика автообновления запускается только на Android через `Capacitor.getPlatform() === 'android'`.
- На web открывается страница релиза, на iOS — APK-ссылка в браузере.
